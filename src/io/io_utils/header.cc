...
void newheader(char *s, register FILE *fp)		/* identifying line of information header */
{
	fputs(HDRSTR, fp);
	fputs(s, fp);
	putc('\n', fp);
...
int headidval(register char *r, register char *s)			/* get header id (return true if is id) */
{
...
int isheadid(char *s)			/* check to see if line is header id */
{
	return(headidval(NULL, s));
}


void printargs(int ac, char **av, register FILE *fp)		/* print arguments to a file */
{
	int  quote;
...
int formatval(register char *r, register char *s)			/* get format value (return true if format) */
{
...
int isformat(char *s)			/* is line a format line? */
{
...
int getheader(FILE  *fp, void (*f)(char *, register struct check *), char *p)		/* get header from file */
{
	char  buf[MAXLINE];
	...
	for ( ; ; ) {
		...
		if (f != NULL)
			(*f)(buf, (struct check *)p);
	}
}

static void mycheck(char  *s, register struct check  *cp)			/* check a header line for format info. */
{
