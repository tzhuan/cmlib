...
int setcolrcor(double (*f)(double, double), double	a2)		/* set brightness correction */
{
	double	mult;
	register int	i, j;
					/* allocate tables */
	if (g_bval == NULL && (g_bval =
			(BYTE (*)[256])malloc((MAXGSHIFT+1)*256)) == NULL)
		return(-1);
...
int setcolrinv(double (*f)(double, double), double	a2)		/* set inverse brightness correction */
{
	double	mult;
	register int	i, j;
					/* allocate tables */
	if (g_mant == NULL && (g_mant = (BYTE *)malloc(256)) == NULL)
		return(-1);
	if (g_nexp == NULL && (g_nexp = (BYTE *)malloc(256)) == NULL)
		return(-1);
...
int gambs_colrs(register COLR *scan, int len)		/* convert gamma bytes to colr scanline */
{
	register int	nexpo;
...
void shiftcolrs(register COLR *scan, register int len, register int adjust)	/* shift a scanline of colors by 2^adjust */
{
	int	minexp;
...
void normcolrs(register COLR *scan, int len, int adjust)	/* normalize a scanline of colrs */
{
	register int  c;
	register int  shift;
