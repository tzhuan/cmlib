...
char *tempbuffer(unsigned len)			/* get a temporary buffer */
{
...
int fwritecolrs(register COLR  *scanline, unsigned int len, register FILE *fp)		/* write out a colr scanline */
{
	unsigned int i, j, beg, cnt=1;
	unsigned int c2;
...
int oldreadcolrs(register COLR  *scanline, int len, register FILE  *fp)		/* read in an old colr scanline */
{
	int  rshift;
	register int  i;
...
int fwritescan(register COLOR  *scanline, int len, FILE  *fp)		/* write out a scanline */
{
	COLR  *clrscan;
	int  n;
	register COLR  *sp;
...
void setcolr(register COLR clr, double r, double g, double b)		/* assign a short color value */
{
	double  d;
	int  e;
...
void colr_color(register COLOR col, register COLR clr)		/* convert short to float color */
{
	double  f;
...
int bigdiff(register COLOR c1, register COLOR c2, double md)			/* c1 delta c2 > md? */
{
	register int  i;
