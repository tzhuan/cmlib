...
bool read_png_(const char *file_name, int *w, int *h,  int *rw, int *rh, unsigned char *pixout[], bool forgl, int *numchannel)
{
	png_structp png_ptr;
	png_infop info_ptr;
	...
	*numchannel=3;
	...
	for (unsigned int k = 0; k < height; k++)
		row_pointers[k] = pixels + k*width*sizeof(unsigned char)*(*numchannel); //for r,g,b,a
...
unsigned char *readPNG(const char *file_name, int &width, int &height, int &depth)
{
	...
	read_png_(file_name, &width, &height, NULL, NULL, &pixels, false, &depth);
...
//	png_colorp palette;
	...
	if (!__comments.empty()) {
		...
		int i=0;
		for (map_iter=__comments.begin(); map_iter!=__comments.end(); map_iter++) {
			text_ptr[i].key = (char *)(map_iter->first.c_str());
			text_ptr[i].text = (char *)(map_iter->second.c_str());
	...
	*/
	...
	for (int k = 0; k < height; k++)
		row_pointers[k] = pixels + k*width*sizeof(unsigned char)*(depth); //for r,g,b,a
